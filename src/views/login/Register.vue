  ...
  methods: {
    handleLogin() {
      this.$refs.loginForm.validate(valid => {
        if (valid) {
          if (this.loginForm.password !== this.loginForm.password2) return this.$message.error('两个密码不一致')
          this.loading = true
          const loginForm = JSON.parse(JSON.stringify(this.loginForm))
          loginForm.password = Md5(loginForm.password)
          loginForm.password2 = Md5(loginForm.password2)
          const nowTime = new Date().getTime()
          if ((nowTime - this.timer) < 2000) return
          this.timer = nowTime
          common.register(loginForm).then(res => {
            this.loading = false
            if (res.code === 0) {
